    ...
    public static void main(String[] args) {
        ...
        while (gameContinue) {
            ...
            for (int i = 0; i < englishWords.length; i++) {
                ...
                if (!correctAnswer[randomIndex].equals("")) { // 오답이 아닌 문제는 건너뛰기
                    ...
                    if (correctCnt != englishWords[randomIndex].length()) { // 틀린 단어들을 해당 인덱스에 저장
                        correctAnswer[randomIndex] = inputWord;
                        anyWrongAnswer = true;
                    } else {
                        correctAnswer[randomIndex] = ""; // 맞은 단어는 오답 목록에서 제거
                    }
            ...
            if(plag == 0){
                for (int oo = rankingIndex; oo < real_rankingScore.length; oo++) {
                    if(rankingIndex!=10){
                         if (real_rankingScore[oo] == 0)
                             real_rankingScore[oo] = rankingScore[oo];  //rankingS를 real로 옮김 복습때문
                ...
                for (int z = 0; z < rankingID.length; z++) {
                    for (int j = z + 1; j < rankingID.length ; j++) { //내림차순 정렬
